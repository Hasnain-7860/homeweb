...
const Contact = () => {
    ...
    const onSubmit = async (event) => {
      ...
      if (data.success) {
        setResult("");
        toast.success("From Submitted Success")
        event.target.reset();
      } else {
  ...
  return (
    <div className='text-center p-6 py-20 lg:px-32 w-full overflow-hidden' id='Contact'>
        ...
        <p className=' text-center text-gray-500 mx-auto max-w-80 mb-8 '>Ready to make a Move? Let's Build Your Future Together </p>
        ...
        <form onSubmit={onSubmit} className='max-w-2xl mx-auto text-gray-600 pt-8' >
            ...
            <button className='border bg-blue-600 text-white px-12 py-2 rounded mb-10 '>{result ?  result:"Send Message" }</button>
        </form>
    </div>
  )
}
